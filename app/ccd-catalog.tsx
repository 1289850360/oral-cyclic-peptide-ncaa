...
type ResearchEvidenceStatus = "linked" | "none";
...
export default function CcdCatalog({ assetPrefix = "" }: { assetPrefix?: string }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([fetch(`${assetPrefix}ccd-unified-structure-catalog.json`), fetch(`${assetPrefix}ccd-unified-polymer-usage.json`)])
  ...
  useEffect(() => {
    ...
    if (!urlQuery) return;
    setQuery(urlQuery); setUsageStatus("all"); setPage(1);
  }, []);
  ...
  const propertyEvidenceCcdCount = useMemo(() => (payload?.records ?? []).filter((record) => propertyEvidenceFor(record).length > 0).length, [payload]);
  ...
  const filtered = useMemo(() => {
    ...
    return payload.records.filter((record) => {
      ...
      const searchable = [
        ...
        ...propertyEvidenceFor(record).flatMap((item) => [item.recordId, item.level, item.name]),
      ...
      return (!normalized || searchable.includes(normalized))
        ...
        && (researchEvidenceStatus === "all" || (researchEvidenceStatus === "linked" ? propertyEvidenceFor(record).length > 0 : propertyEvidenceFor(record).length === 0))
  ...
  }, [payload, query, category, usageStatus, targetProperty, componentClass, researchEvidenceStatus, usageById]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const safePage = Math.min(page, totalPages);
  const shown = filtered.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
  const changeQuery = (value: string) => { setQuery(value); if (value.trim()) setUsageStatus("all"); setPage(1); };
  ...
  const changeComponentClass = (value: "all" | ComponentClass) => { setComponentClass(value); setUsageStatus("all"); setPage(1); };
  const changeResearchEvidenceStatus = (value: "all" | ResearchEvidenceStatus) => { setResearchEvidenceStatus(value); setUsageStatus("all"); setPage(1); };

  return (
    <section className="ccd-catalog-section ccd-catalog-page" id="ccd-catalog" aria-labelledby="ccd-catalog-title">
      ...
      {/* 旧版92条深度审核浏览器已停用；历史CSV仅作为审计资料保留。
