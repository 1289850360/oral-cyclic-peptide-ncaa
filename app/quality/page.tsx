...
const qualityMetrics = [
  ...
  { value: "1,845", label: "已确认非天然氨基酸／残基", note: "当前身份待确认记录为0" },
    ...
    <section className="quality-page">
      ...
      <div className="quality-layout">
        <div className="quality-main">
          <section><div className="detail-section-title"><span>01</span><h2>当前正式版本</h2></div><dl className="quality-release"><div><dt>版本</dt><dd>Version 5.2 · 当前发布版</dd></div><div><dt>身份复核日期</dt><dd>2026-09-02</dd></div><div><dt>主记录Schema</dt><dd>ocpr-master-record-v1</dd></div><div><dt>完整CCD对照</dt><dd>wwPDB CCD · 2026-08-29</dd></div></dl><div className="quality-changelog"><h3>本版本变化</h3><ul><li>2,065条CCD结构继续使用统一主记录。</li><li>CCD 96Z经RCSB结构身份、AzAla制备论文和大环肽编入实验核验后归入已确认残基。</li><li>当前分类为1,845条已确认非天然氨基酸／非标准残基、176条特殊结构、44条排除项，身份待确认记录为0。</li><li>身份、PDB使用、合成可用性和性质证据继续分开记录。</li></ul></div></section>
          ...
          <section><div className="detail-section-title"><span>03</span><h2>构件身份分类</h2></div><div className="quality-component-classes"><article className="component-evidence-reviewed-residue"><b>1,845 · 已确认非天然氨基酸／非标准残基</b><p>包括独立非天然氨基酸、单一标准氨基酸母体的修饰残基及明确的直接衍生物；用途特殊不否定身份。</p></article><article className="component-special-reference"><b>176 · 特殊用途结构</b><p>多残基融合、成熟发色团、反应状态、还原端或完整肽模拟构件等不作为一个普通氨基酸理解。</p></article><article className="component-excluded-nonmonomer"><b>44 · 非单体／明确排除</b><p>缺少单一氨基酸骨架，或属于完整药物、短肽和其他非单体结构。</p></article></div><p className="detail-boundary">三类互不重叠，合计2,065条；当前没有身份待确认记录。身份分类不表示这些残基都可以买到、都能直接用于SPPS，或都能改善口服性质。</p></section>
          ...
          <section><div className="detail-section-title"><span>05</span><h2>质量控制与已知缺口</h2></div><ul className="quality-check-list"><li><strong>已完成：</strong>2,065条记录全部完成PDB聚合物序列查询，三种出现状态合计无缺失、无重复。</li><li><strong>已完成：</strong>1,681个官方肽链连接型候选及CCD 96Z全部完成化学身份复核。</li><li><strong>已完成：</strong>早期分类中“用途特殊”覆盖身份判断的问题已经纠正。</li><li><strong>已完成：</strong>661条历史资料进入CCD—证据审计表；正式性质证据库显示46条实验记录。</li><li><strong>进行中：</strong>1,845条已确认非天然氨基酸／非标准残基中首批20条完成合成可用性审核。</li><li><strong>仍需完善：</strong>完整扫描46,185个NON-POLYMER组分中的氨基酸样结构，并继续增加性质实验文献。</li></ul></section>

          <section><div className="detail-section-title"><span>06</span><h2>引用与使用边界</h2></div><div className="citation-box"><span>推荐引用格式</span><p>口服环肽非天然残基证据库. Version 5.2. 2026-09-02. https://1289850360.github.io/oral-cyclic-peptide-ncaa/</p></div><p>引用单条残基时，请优先使用详情页中的OCPR永久编号、CCD编号或OCP-X内部概念标识，并保留原始来源。RCSB、PDBe、论文及专利内容仍遵循各自来源的许可和引用要求。</p><p>本站尚未声明统一的再分发许可证；在维护者确认正式许可前，整理内容按研究引用使用，不应移除来源或证据边界后重新发布。</p></section>
        </div>

        <aside className="quality-sidebar">
          <section><span>机器读取</span><a href="../api/v1/residues">46条性质证据 API</a><a href="../api/v1/compounds">CCD结构目录 API</a><a href="../ccd-unified-structure-catalog.csv">统一结构目录 CSV</a><a href="../ccd-ncaa-complete-coverage-audit.json">完整CCD覆盖审计</a><a href="../natural-parent-mapping-audit.json">天然母体映射审计</a><a href="../natural-parent-content-consistency-audit.json">修饰与证据一致性审计</a><a href="../natural-parent-name-structure-semantic-audit.json">名称—结构语义审计</a></section>
