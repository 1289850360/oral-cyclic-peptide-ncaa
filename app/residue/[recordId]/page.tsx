  ...
  return <main>
    ...
    <article className="residue-detail-page">
      ...
      <header className="detail-hero">
        ...
        <dl className="detail-identity-grid"><div><dt>永久编号</dt><dd>{record.id}</dd></div><div><dt>记录版本</dt><dd>V{record.recordVersion}</dd></div><div><dt>最近审核</dt><dd>{record.reviewedAt}</dd></div><div><dt>CCD映射</dt><dd>{structures.length ? structures.map((item) => item.ccd).join(" / ") : "暂无唯一映射"}</dd></div></dl>
      </header>

      <div className="detail-layout">
        <div className="detail-main-column">
          <section className="detail-section"><div className="detail-section-title"><span>01</span><h2>结构身份</h2></div>{structures.length ? <div className="detail-structures">{structures.map((structure) => <a href={ccdEntryUrl(structure.ccd)} target="_blank" rel="noreferrer" key={structure.ccd}><img src={ccdImageUrl(structure.ccd)} alt={`${structure.label}二维结构`} /><div><b>CCD {structure.ccd}</b><span>{structure.label}</span><small>打开RCSB原始记录 ↗</small></div></a>)}</div> : <div className="detail-empty"><strong>暂无唯一CCD结构</strong><p>{structureNotesByEnglish[record.english] ?? "当前记录尚未找到与名称和立体化学完全对应的CCD单体。"}</p></div>}</section>

          <section className="detail-section"><div className="detail-section-title"><span>02</span><h2>研发定位与性质假设</h2></div><p className="detail-lead">{record.effect}</p><dl className="detail-definition-list"><div><dt>优先替换位置</dt><dd>{design.replace}</dd></div><div><dt>主要目标</dt><dd>{design.goal}</dd></div><div><dt>限制条件</dt><dd>{design.avoid}</dd></div></dl><p className="detail-boundary">性质方向来自类别规律或现有骨架证据，不能替代目标环肽中的同骨架替换实验。</p></section>
