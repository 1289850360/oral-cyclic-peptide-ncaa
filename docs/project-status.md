...
- 2,065 条记录已完成统一结构身份分类：已确认非天然氨基酸 1,316 条、特殊用途结构 446 条、非单体／明确排除 303 条；三类合计 2,065，不重复计数。PDB聚合物序列使用状态单独统计，不再作为结构身份类别。
- 1,805 条由结构规则生成的补充审核队列，分为 P0（6）、P1（45）、P2（484）和 P3（1,270）。
- 653 条经过去重合并的研发证据记录；602 条可以映射到 CCD，共关联 603 个 CCD 结构。其余 51 条属于构型未唯一确定、特定取代构件或骨架改造策略，暂无唯一 CCD 可对应。
...
- 研发证据库只收录具有实验性质终点的28条正式记录；其余历史资料保留在审计对应表中，不参与正式性质证据计数。
...
2. 对剩余277条身份不明确记录补查原始论文、专利、保护形式、供应或公开合成路线。
