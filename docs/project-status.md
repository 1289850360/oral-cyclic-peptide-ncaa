...
- 2,065 个可检索 CCD 结构记录：1,687 个结构筛选核心、92 个研发证据关联 CCD，以及 286 个尚待合成和环肽证据核查的条件候选。
- 2,065 条记录已经按构件身份做互斥分类：人工确认肽／宏环构件 105 条、PDB 肽中出现但单体资料待补 168 条、特殊构件／结构参考 294 条、氨基酸样待验证候选 1,269 条、非单体／明确排除 229 条；五类合计 2,065，不重复计数。
- 1,805 条由结构规则生成的补充审核队列，分为 P0（6）、P1（45）、P2（484）和 P3（1,270）。
- 139 条经过去重合并的研发证据记录；其中 125 条具有唯一 CCD 映射，现已全部覆盖在结构目录中，其余 14 条属于构型未唯一确定、组合构件或骨架改造策略，暂无单一 CCD 可对应。
...
- 105条人工确认构件的合成可用性审核已启动；第一批20条已核查商业保护形式、Fmoc-SPPS兼容性、偶联建议和主要风险，其中11条可从标准Fmoc流程起步，9条需要强化偶联或特别监测。供应商目录不作为实时库存承诺。
- 168条“PDB肽中出现”记录已完成代表PDB标题语境初筛：29条直接环肽／宏环线索、6条混合语境、4条其他约束肽线索、2条明确线性线索、127条仅凭标题不能判断。该结果是机器排序线索，不是环化拓扑人工确认。
- 1,269条待验证候选已完成结构字段二次分层：957条较像独立偶联单体、53条特殊反应基团候选、5条强非普通单体信号、254条信息不足；结果不推定供应、SPPS/LPPS或环肽使用成立。
- 新增14条实验骨架记录，按完整环肽的序列、环化方式、实验终点、数值、体系和归因边界编目。
...
```bash
npm run build:scaffold-evidence
```

它会生成 `public/cyclic-scaffold-evidence.json` 和 CSV。定量结果只在原始实验体系内解释，不跨体系直接排序。

## 后续完成顺序

1. 优先人工复核35条含直接或混合环肽标题线索的PDB记录，确认实体序列和真实环化拓扑。
2. 优先核查957条“较像独立单体”候选中的保护形式、供应或公开合成路线，再决定是否升级为可用单体。
3. 继续扩充实验骨架库；只有存在同骨架实验时，才增加具体性质方向和定量归因。
4. 对优先候选逐条补论文、专利、供应商或合成单体来源。
5. 每次发布前构建网站，并在桌面、移动端检查筛选、分页、图片和下载资源。
